...
def distplots(data,var,height=600):
    x1 = data.loc[data['PRED'] == 0, var].tolist()
    x2 = data.loc[data['PRED'] == 1, var].tolist()
    x=data.loc[idx][var]
...
with st.expander("Plus d'informations"):
    ...
    #var 1
    st.plotly_chart(distplots(clients,var_1), use_container_width=True)
    #var 2
    st.plotly_chart(distplots(clients,var_2), use_container_width=True)
